...
export default async function (client: BernardClient, interaction: ButtonInteraction) {
    ...
    if (interactionTcheck.substring(31, 13) !== interactionUserId && interactionTcheck.split('-')[1] !== interactionUserId)
        return interaction.reply({content: `${error} | You are **not part** of the game!`, ephemeral: true});

    let requestGameUser = interactionTcheck.substring(31, 13);
    ...
    gridBox[0][1] = interactionUserId === morpionConfig.firstPlayer ? "⭕" : "❌";
    ...
    if (gridBox[0][1] === gridBox[0][0] && gridBox[0][1] === gridBox[0][2]){
        await interaction.update({content: `${messageVictory}`, components: [], embeds: []})
        return morpionConfig.delete();
    }
    else if (gridBox[0][1] === gridBox[1][1] && gridBox[0][1] === gridBox[2][1]){
        await interaction.update({content: `${messageVictory}`, components: [], embeds: []})
        return morpionConfig.delete();
    }
