...
export default async function (client: BernardClient, interaction: ButtonInteraction, language: any) {
    ...
    gridBox[2][2] = interactionUserId === morpionConfig.firstPlayer ? "⭕" : "❌";
    ...
    if (gridBox[2][2] === gridBox[2][0] && gridBox[2][2] === gridBox[2][1]){
        await interaction.update({content: `${messageVictory}`, components: [], embeds: []})
        return morpionConfig.delete();
    }
    else if (gridBox[2][2] === gridBox[1][1] && gridBox[2][2] === gridBox[0][0]){
        await interaction.update({content: `${messageVictory}`, components: [], embeds: []})
        return morpionConfig.delete();
    }
    else if (gridBox[2][2] === gridBox[1][2] && gridBox[2][2] === gridBox[0][2]) {
        await interaction.update({content: `${messageVictory}`, components: [], embeds: []})
        return morpionConfig.delete();
    }
