...
export default async function (client: BernardClient, interaction: CommandInteraction, language: any) {
    ...
    switch(action) {
        case "user":
            ...
            if (!userOption) return interaction.replyErrorMessage(client, language("SPECIFY_USER"), true);
            ...
            let embed = new MessageEmbed()
                ...
                .addFields(
                    {name: language("MONEY_VALUE"), value: `${money} ${memberConfig.money}`, inline: true},
                    {name: language("BANK_VALUE"), value: `${bank} ${memberConfig.bank}`, inline: true},
        ...
        case "add":
            ...
            if (!montant) return interaction.replyErrorMessage(client, language("SPECIFY_MONTANT"), true);
            if (montant > userToAddConfig.money) return interaction.replyErrorMessage(client, language("MONTANT_ENOUGH"), true);
            ...
            return interaction.replySuccessMessage(client, language("MONTANT_ADDED").replace('%montant%', montant).replace('%emoji%', money), false);
            break;
        case "remove":
            let userToRemoveConfig: any = await find(interaction.guild!.id, interaction.user.id);
            if (!montant) return interaction.replyErrorMessage(client, language("SPECIFY_MONTANT"), true);
            if (montant > userToRemoveConfig.bank) return interaction.replyErrorMessage(client, language("MONTANT_ENOUGH"), true);
