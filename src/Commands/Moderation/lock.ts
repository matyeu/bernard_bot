...
export default async function (client: BernardClient, interaction: CommandInteraction, language: any) {
    ...
    if (state === 1) {
        ...
        await interaction.replySuccessMessage(client,language("CHANNEL_BEING").replace('%state%', "locked"), true)
        return interaction.channel?.send({content: language("CHANNEL_STATE").replace('%emoji%', channelLock).replace('%state%', "locked")})

    } else {
        await channel.permissionOverwrites.edit(everyone.id, {SEND_MESSAGES: null})

        await interaction.replySuccessMessage(client,language("CHANNEL_BEING").replace('%state%', "unlocked"), true)
        return interaction.channel?.send({content: language("CHANNEL_STATE").replace('%emoji%', channelUnlock).replace('%state%', "unlocked")});
    }
